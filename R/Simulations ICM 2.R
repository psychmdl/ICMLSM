...
ICM_Fit2 <- function(
  ...
  for(i in 1:Num_Sims){
    ...
    L2Strength = runif(1, 1, 10)
    L1_Activation = runif(1, 1, 10)
    L2_Activation = runif(1, 1, 10)
    c = runif(1, .01, 5)
    x = ICMSimulation(Simulations = 1, Participants = 1, Num_SubBlocks = 8, Trial_Comparisons = c(1,2,3,4,5,6), Comp = competition, h1 = h, h2 = h, L1_Strength = L1Strength, L2_Strength = L1Strength, NoiseMu = .0001, NoiseTau = .0001, NoiseSigma = .001, L1_Activation = L1_Activation, L2_Activation = L2_Activation)
    ...
    crecord[i] <- c
    L1ActR[i] <- L1_Activation
    L2ActR[i] <- L2_Activation

  }
  df1 <- data.frame(cor1, cor2, hrecord, L1Record, comprecord, chi1, pval1, chi2, pval2, RMS1, RMS2, crecord, L1ActR, L2ActR)
