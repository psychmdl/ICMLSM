...
LSM_Fit <- function(
  ...
  for(i in 1:Num_Sims){
    ...
    cor1 <- vector()
    cor2 <- vector()
    x = LSMSimulation(Simulations = 1, Participants = 2, Num_SubBlocks = 8, Trial_Comparisons = c(1,2,3,4,5,6), Comp = competition, h1 = h, h2 = h, L1_Strength = L1Strength, L2_Strength = L1Strength)
