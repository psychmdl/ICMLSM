...
LSM_Fit_Abs <- function(
  ...
  RMS2 <- vector()
  for(i in 1:Num_Sims){
    competition = runif(1, .10, .50)
    L1Strength = runif(1, 1, 10)
    h = runif(1, .01, 10) #Language Activation parameter on switch trials = h*3
    x = LSMSimulation_Abs(Simulations = 1, Participants = 1, Num_SubBlocks = 8, Trial_Comparisons = c(1,2,3,4,5,6), Comp = 3.4, h1 = h, h2 = h, L1_Strength = L1Strength, L2_Strength = L1Strength, NoiseMu = .0001, NoiseTau = .0001, NoiseSigma = .001, p1 = competition)
